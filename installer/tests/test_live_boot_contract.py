...
class LiveBootContractTests(unittest.TestCase):
    ...
    def test_plymouth_ready_waits_for_first_qt_frame(self):
        unit = INSTALLER_SERVICE.read_text(encoding="utf-8")
        installer = INSTALLER_MAIN.read_text(encoding="utf-8")
        ...
        self.assertIn('QStringLiteral("/usr/bin/systemd-notify")', installer)
        self.assertIn('QStringLiteral("/usr/bin/systemd-notify")', repair)
        self.assertIn('QStringLiteral("--ready")', installer)
        self.assertIn('QStringLiteral("--ready")', repair)
        self.assertNotIn('startDetached(QStringLiteral("/usr/lib/meoarch/meo-boot-status")', repair)
