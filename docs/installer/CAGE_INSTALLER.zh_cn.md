...
正常 Live 启动保持尽量简洁。Plymouth 使用纯黑背景，只显示居中的 MeoArch
Logo，不再显示旧的转圈、进度条、页脚或普通状态文字；如果启动失败，仍可切换
到错误诊断信息。

接近图形环境启动时，Plymouth 的 Logo 会逐渐淡到黑色。Cage 启动安装器后，
`Main.qml` 会先显示相同的黑底 Logo 一小段时间，再把这一层淡出到真正的安装
界面。这样可以减少 Plymouth 和 Cage 交接时出现亮色壁纸闪烁或未绘制画面的情况。

这里不强行做假的模糊效果。以后如果加入经过确认的少量柔和色块，纯黑仍然作为
可靠的回退方案。
