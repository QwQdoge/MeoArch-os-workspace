...
qcow2 disk, user-mode NAT and a virtio virtual GPU. KVM is used when available;
QEMU TCG is the fallback.

Create a disposable disk below the test-run artifacts directory:

```bash
export MEOARCH_RUN_DIR="$PWD/artifacts/test-runs/manual"
...
./scripts/acceptance/60-boot-live.sh \
  "$PWD/out/meoarch-os-YYYY.MM.DD-x86_64.iso" \
...
For installation, select only the 64 GiB QEMU virtio disk. After a clean
